    ...
    def _update_model_kwargs_for_generation(self, outputs, model_kwargs, is_encoder_decoder=False, num_new_tokens=1):
        ...
        if outputs.hidden_states is not None:
            ...
            if codec_ids is not None:
                callbacks = getattr(self, "_streaming_callbacks", {})
                callback = callbacks.get("__engine__") or callbacks.get(threading.current_thread().ident)
                if callback is not None:
                    callback.on_codec_generated(codec_ids)
...
class Qwen3TTSForConditionalGeneration(Qwen3TTSPreTrainedModel, GenerationMixin):
    ...
    def generate(
        ...
        **kwargs,
        ...
        for index, (input_id, language, speaker) in enumerate(zip(input_ids, languages, speakers)):
            ...
            spk_is_dialect = getattr(self.config.talker_config, "spk_is_dialect", None) or {}
            if (
                language.lower() in ["chinese", "auto"]
                and speaker != ""
                and speaker is not None
                and speaker.lower() in spk_is_dialect
                and spk_is_dialect[speaker.lower()] is not False
            ):
    ...
    async def generate_streaming(
        ...
        repetition_penalty: float = 1.05,
        chunk_size: int = 24,
        min_first_chunk: int = 12,
        **kwargs,
    ):
        """Streaming variant of generate() that yields TTSCodecChunk objects."""
        import asyncio
        import concurrent.futures

        from .tts_streamer import TTSCodecCallback

        logger.info("generate_streaming called with %d inputs", len(input_ids))

        try:
            loop = asyncio.get_running_loop()
        except RuntimeError as e:
            logger.error("No running event loop: %s", e)
            return

        callback = TTSCodecCallback(chunk_size=chunk_size, min_first_chunk=min_first_chunk)
        callback.set_event_loop(loop)
        logger.info("Callback created and event loop set")

        if not hasattr(self.talker, "_streaming_callbacks"):
            self.talker._streaming_callbacks = {}
        ...
        for index, (input_id, language, speaker) in enumerate(zip(input_ids, languages, speakers)):
            if voice_clone_spk_embeds is None:
                if speaker == "" or speaker is None:
                    ...
                    spk_id_dict = getattr(self.config.talker_config, "spk_id", None) or {}
                    if speaker.lower() in spk_id_dict:
                        spk_id = spk_id_dict[speaker.lower()]
                        speaker_embed = self.talker.get_input_embeddings()(
                            torch.tensor(
                                spk_id,
                                device=self.talker.device,
        ...
        logger.info("About to create executor and start generation thread")

        def run_generation():
            thread_id = threading.current_thread().ident
            self.talker._streaming_callbacks[thread_id] = callback
            logger.info("run_generation: registered callback for thread %s", thread_id)

            try:
                logger.info("run_generation: starting talker.generate")
                ...
                logger.info("run_generation: talker.generate completed, calling finalize")
                callback.finalize()
                return result
            except Exception as e:
                logger.error("run_generation: exception occurred: %s", e)
                callback.signal_error(e)
                raise
            finally:
                self.talker._streaming_callbacks.pop(thread_id, None)

        executor = concurrent.futures.ThreadPoolExecutor(max_workers=1)
        gen_future = loop.run_in_executor(executor, run_generation)
        logger.info("Executor task scheduled, starting async iteration")

        try:
            async for chunk in callback:
                logger.info("Received chunk %d", chunk.chunk_index)
                yield chunk
        finally:
            try:
                await gen_future
            except Exception as e:
                logger.error("generate_streaming: gen_future exception: %s", e)
